...
We denote by $B_\mot G := L_\mot BG$, and we have seen the following: (\todo fix this part)

\begin{proposition} If $X$ is a scheme, then
\begin{align*}
    \pi_0 \Map_{\Shv_\Nis(\Sm_k)} \left( h_X, B_\Nis G \right) \cong H^1_\Nis(X,G).
\end{align*}
\end{proposition}
...
\begin{theorem} \cite[Theorem~3]{ABH-Freudenthal} (Asok--Bachmann--Hopkins) Murthy's splitting conjecture holds in characteristic zero.
\end{theorem}
