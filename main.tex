...
Thank you to Howard Beck, Merrick Cai, Juan Fernando Valdes, Jake Levinson, Jiantong Liu, Isabel Longbottom, Sof\'ia Marlasca Aparicio, Matthew Niemiro, Natalie Stewart, Maria Yakerson, and Qi Zhu for their suggestions and changes to these notes.

Thank you to Maxine Calle, Connor Cassady, David Chan, Liam Keenan, and Maximilien P\'eroux for letting me bother them about technical non-motivic questions.
...
\subsection{Overview} What sorts of things about a ring $R$ are still true when we move to the polynomial ring $R[t]$? In other words, what sorts of things about $R$ can't be varied in a 1-parameter family? Let's give a ton of examples! Don't stress if not all of the words are familiar, we'll break down what's happening here over the course of the semester, this is just motivation.
...
\end{aligned}
\end{equation}
\textbf{Serre's Problem}: Is every finitely generated projective $k[t_1, \ldots, t_n]$-module free?

Serre's problem is equivalent to the statement that \autoref{eqn:extn-scalars} is essentially surjective. Equivalently under the Serre correspondence, this is the same as saying that every algebraic vector bundle on affine space $\A^n_k$ is trivial.
...
Show that \textit{every} sheaf of groups $\mathcal{G}$ is of the form $\Aut_\mathcal{F}(x)$ for some sheaf of categories $\mathcal{F}$.
\end{exercise}
...
\subsection{Why we like representability of torsors in homotopy theory}


The fact that principal $G$-bundles are representable in homotopy theory (in the sense of \autoref{eqn:representability-torsors-homotopy-theory}) has a number of applications. We list three here, to motivate what tools we will try to build when studying torsors in algebraic topology with motivic methods.
 First off:
\begin{itemize}
