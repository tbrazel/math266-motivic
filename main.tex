...
\begin{definition} Let $X$ be a compact Hausdorff topological space and $G$ a group. Then a \textit{principal $G$-bundle} (or we might just say a $G$-torsor) is a fiber bundle $\pi \colon Y \to X$ so that $G$ acts freely and transitively on the fibers.\footnote{\todo this is wrong, add in local trivialization condition}
\end{definition}
...
\begin{align*}
    \left[ X, \mathbb{G}_m \right]_{\A^1} = \mathbb{G}_m(X) = H^0(X,\O_X^\times)
\end{align*}
for any $X\in \Sm_S$.
\end{example}
...
Much of this can be found in \cite[\S7.1]{GilleSzamuely}.
...
\end{align*}
which concludes the proof. If $q$ has odd exponential characteristic, we can find two non-squares in $\mathbb{F}_q$ which sum to 1 (c.f. \cite[1.3.6]{GilleSzamuely}). This gives us $x^k + x^\ell = 1$ for $k,\ell$ odd. Hence we get
\begin{align*}
...
\begin{enumerate}
    ...
    \item Rost cycle modules were used famously in the proof of the Milnor conjecture. Moreover over a perfect field, the category of Rost cycle modules is equivalent to the heart of the category of Voevodsky motives in the homotopy $t$-structure\footnote{\todo add reference to D\'eglise's modules de cycles et motifes mixtes}
\end{enumerate}
\end{remark}
...
\begin{definition} \cite[p.~11]{SGA6} If $X$ is a scheme, we say a chain complex of $\O_X$-modules is \textit{perfect} if it is quasi-isomorphic to a bounded chain complex in $D(X)$. We denote by $\Perf(X)$ the category of perfect complexes over $X$.
\end{definition}
