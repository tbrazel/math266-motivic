...
\usepackage{minitoc}
% to remove useless warning messages from minitoc
\usepackage{silence}
...
\begin{enumerate}
    \item The Fall 2024 Thursday seminar is on the motivic Wilson space hypothesis, so we're hoping this class will contrast nicely and provide some foundational background in techniques in motivic obstruction theory.
...
Thank you to Howard Beck, Merrick Cai, Juan Fernando Valdes, Jake Levinson, Jiantong Liu, Isabel Longbottom, Sof\'ia Marlasca Aparicio, Matthew Niemiro, Natalie Stewart, Maria Yakerson, and Qi Zhu for their suggestions and changes to these notes. Thank you to Maxine Calle, Connor Cassady, David Chan, Liam Keenan, and Maximilien P\'eroux for letting me bother them about technical non-motivic questions. Finally, a special thank you to Aravind Asok, Tom Bachmann, Elden Elmanto, Niels Feld, Mike Hopkins, and Matthias Wendt for answering some motivic questions as I was writing these notes.

\section{Introduction}

\subsection{Overview} What sorts of things about a ring $R$ are still true when we move to the polynomial ring $R[t]$? In other words, what sorts of things about $R$ can be varied in a 1-parameter family without changing? Let's give a ton of examples! Don't stress if not all of the words are familiar, we'll break down what's happening here over the course of the semester, this is just motivation.
...
\end{aligned}
\end{equation}
\textbf{Serre's Problem}: Is every finitely generated projective $k[t_1, \ldots, t_n]$-module free? In other words, is the functor \autoref{eqn:extn-scalars} essentially surjective? Or, in other other words, under the Serre correspondence, is every algebraic vector bundle on affine space $\A^n_k$ trivial?

\textbf{Answer}: Yes \cite{Quillen76,Suslin76}. Quillen actually proved more-- for $R$ a PID, he proved that the every finitely generated projective $R[t]$-module is extended from an $R$-module.\footnote{%
...
\end{align*}
is essentially surjective? This is \textit{still open}.

\textbf{Fundamental Theorem of Algebraic $K$-Theory} \cite{Quillen-K1}: For $R$ regular Noetherian, we have that $R \to R[t]$ induces an equivalence\footnote{%
...
\begin{theorem}[{Harder's Theorem, \cite[VII.3.13]{Lam-SerresProblem}}] Let $k$ be a field. Then every inner product space over $k[t]$ is extended from an inner product space over $k$.
\end{theorem}
...
%     \item we have access to obstruction theory for classifying torsors over affine varieties --- recent success in this direction is Asok--Bachmann--Hopkins' resolution of Murthy's conjecture \cite{ABH-Freudenthal}
...
Some cheatsheets can be found throughout. I also include them here in the intro for ease of reference.

\input{cheatsheets/topologies}

\input{cheatsheets/contraction}

\input{cheatsheets/higher-cats}

\chapter{Torsors}
\minitoc
\section{Torsors}

\begin{assumption} For this lecture, every time we say ``cover'' you can assume we are working with Zariski covers, or even open covers of topological spaces, since the intuition will be the same and the results here will be mostly identical. We'll go deeper into Grothendieck topologies later in these notes.
\end{assumption}
...
Observe that there are two types of $G$-torsors --- sets  that look like $G/e$, and the empty set. Depending on the convention, we might want to exclude the case of the empty set by including $X \ne \emptyset$ in the definition.

\begin{intuition} A nonempty $G$-torsor is like a group $G$ which has remembered its multiplication but forgotten its identity. Any choice of basepoint $x\in X$ yields a canonical bijection $G \xto{\sim} X$ by sending $g \mapsto g\cdot x$.
\end{intuition}
...
\begin{example} For any group scheme $G$, we will refer to $G$\textit{-torsors}, by which we mean torsors for the representable functor $\Hom_\Sch(-,G)$.
\end{example}
...
\end{proposition}
\begin{proof} A trivial torsor admits a global section, namely the identity. Conversely a choice of global section is a choice of trivialization.
\end{proof}




\begin{theorem}\label{thm:every-morphism-in-torsG-is-an-iso} 
Every morphism in $\Tors_G(X)$ is an isomorphism.
\end{theorem}
...
\begin{example}
The associated frame bundle of an algebraic vector bundle is a $\GL_n$-torsor in the Zariski topology.\footnote{Or Nisnevich or \'etale topology --- see \Cref{cor:h1-gln-hilbert90}.}
\end{example}
...
Let's do this, by first considering an alternative perspective on what a torsor is (the exposition that follows borrows a lot from Alex Youcis' excellent note on torsors \cite{Youcis}).
...
\end{align*}
%
\textbf{Q}: Does this functor necessarily reflect isomorphisms?\footnote{A functor which reflects isomorphisms is called \textit{conservative}.}

\begin{example} Let $\mathcal{F}$ be the functor sending $U$ to the category of line bundles over $U$. If the cover is picked to be appropriately small, then all line bundles are isomorphic to the trivial line bundle over $U_i$, but they need not be isomorphic globally as line bundles over $U$.
\end{example}
...
\begin{theorem}\label{thm:cech-cohom-measures-an-LGP} 
There is a bijection between $\check{H}^1(U,\Aut_{\mathcal{F}}(x))$ and isomorphism classes of objects $y\in \mathscr{F}(U)$ which are locally isomorphic to $x$.
\end{theorem}
...
\end{theorem}
\begin{proof}[Sketch] Let $\mathcal{F}$ be a $\mathcal{G}$-torsor, and pick $s_i \in \mathcal{F}(U_i)$ for each $i$. Then on the overlap $U_i \times_U U_j$, we have that $s_i$ and $s_j$ differ by a unique element $g_{ij}\in \mathcal{G}(U_i \times_U U_j)$. We run basically an identical argument to the proof of \Cref{thm:cech-cohom-measures-an-LGP}.
\end{proof}
...
Show that \textit{every} sheaf of groups $\mathcal{G}$ is of the form $\Aut_\mathcal{F}(x)$ for some sheaf of categories $\mathcal{F}$. (Hint: first prove \Cref{thm:every-morphism-in-torsG-is-an-iso})
\end{exercise}
...
\begin{question} In \autoref{set:representable-torsor-and-group-scheme}, what does it mean in this case for $\mathcal{F}$ to be a $\mathcal{G}$-torsor?
\end{question}
...
\[ \begin{tikzcd}
     & Y\dar\\
    U_i\rar\ar[ur,"s_i" above left] & X.
\end{tikzcd} \]
...
\begin{enumerate}
    \item the map $G \times_X Y \to Y \times_X Y$ is an isomorphism of $X$-schemes, and
...
\end{tikzcd} \]
Hence we have a trivialization, and therefore a principal $\Gal(L/k)$-bundle (a $\Gal(L/k)$-torsor).
\end{proof}
...
\subsection{Interlude: representability of torsors in homotopy theory}

It's a very reasonable goal to ask for any tools that could help us try to classify torsors. A natural idea, by analogy, is to look to homotopy theory, where we have a suite of tools for studying torsors.

\begin{definition} Let $X$ be a compact Hausdorff topological space and $G$ a group. Then a \textit{principal $G$-bundle} (or we might just say a $G$-torsor) is a fiber bundle $\pi \colon Y \to X$ so that $G$ acts freely and transitively on the fibers, \textit{and} there exists an open cover $\left\{ U_i \to X \right\}$ over which $\pi$ is trivialized (contrast this with \autoref{prop:principal-G-bundle}).
\end{definition}
...
\begin{equation}\label{eqn:representability-torsors-homotopy-theory}
\begin{aligned}
...
\begin{theorem}[{\cite{Samelson41}}]
\label{thm:fibration-from-subgp-of-liegp}
If $H \le G$ is a compact subgroup of a Lie group, then $G \to G/H$ is a Serre fibration and principal $H$-bundle.
\end{theorem}

\begin{corollary} 
\label{cor:fiber-sequences-from-subgps-lie-gps}
In the setting of \autoref{thm:fibration-from-subgp-of-liegp}, we obtain fiber sequences
\begin{align*}
    H \to G \to G/H \\
    G/H \to BH \to BG.
\end{align*}
...
\begin{example} Applying \autoref{cor:fiber-sequences-from-subgps-lie-gps} to the inclusions inclusions $O(n) \subseteq O(n+1)$ and $U(n) \subseteq U(n+1)$, we obtain fiber sequences
\begin{align*}
    S^{2n+1} \to \BU(n) \to \BU(n+1) \\
    S^n \to \BO(n) \to \BO(n+1).
\end{align*}
\end{example}
Leveraging these fiber sequences is one way to prove Bott periodicity.


\textbf{Why we like representability of torsors in homotopy theory}: The fact that principal $G$-bundles are representable in homotopy theory (in the sense of \autoref{eqn:representability-torsors-homotopy-theory}) has a number of applications. We list three here, to motivate what tools we will try to build when studying torsors in algebraic topology with motivic methods.
 First off:
\begin{itemize}
    \item Suppose we have two torsors $f,g \colon X\to BG$ and we want to prove they are not isomorphic. If we can find any cohomology theory $E^\ast$ and any class $c\in E^\ast(BG)$ so that $f^\ast(c) \ne g^\ast(c)$ in $E^\ast(X)$, this means that $f$ and $g$ correspond to non-isomorphic torsors. This the basic idea of characteristic classes.
\end{itemize}
...
\begin{notation} Let $\mathscr{C}$ be a category. Then we refer to a functor $\DDelta \to \mathscr{C}$ as a \textit{cosimplicial object} in $\mathscr{C}$, and a functor $\DDelta^\op \to \mathscr{C}$ as a \textit{simplicial object} in $\mathscr{C}$.\footnote{People say things like ``a simplicial object in $\mathscr{C}$'' although we should clarify this sentence doesn't quite parse -- it is an object of $\Fun(\DDelta^\op),\mathscr{C})$ not an object of $\mathscr{C}$, just to clarify.}
\end{notation}
...
\begin{exercise} Convince yourself that every map in $\DDelta$ factors into face and degeneracy maps, and therefore any simplicial object $X_\bullet$ in $\mathscr{C}$ can be described by the data of $X_n$ for each $n$, and its face and degeneracy maps. The intuition is that, by only describing $X_n$ and the face and degeneracy maps, we have \emph{compressed} the data of a simplicial object in a lossless way.\footnote{%
...
\end{tikzcd} \]
where each $X_i$ is smooth and projective. There is a cohomological descent spectral sequence computing the cohomology of $X$ in terms of the cohomology of the $X_n$'s. This dates back to SGA 4. This replacement is the core idea of \textit{resolution of singularities}. Deligne uses these simplicial methods to endow singular varieties with Hodge structures, so-called \textit{mixed Hodge structures} \cite{Hodge3}.
\end{example}
...
\begin{example}[The \v{C}ech nerve $N_\bullet(U)$] A simplicial object can conveniently encode the data of an open cover, its intersections, triple intersections, etc.\footnote{%
...
\begin{enumerate}
    ...
    \item It is not true (even in simplicial sets) that this is an equivalence relation. Nevertheless it generates one, so when we say ``up to simplicial homotopy'' we often mean with respect to the transitive closure of this relation.
\end{enumerate}
\end{remark}

\begin{theorem}
Geometric realization preserves products.
\end{theorem}
\begin{proof}
By the co-Yoneda lemma, we can write any $X,Y\in \sSet$ as a colimit of representables, and since colimits in spaces $\mathcal{S}$ are universal (in particular finite products commute with finite colimits), the product $X \times Y$ distributes over the colimits, so $X \times Y$ can be written as a colimit of products of representable. Hence it suffices to restrict to the case where $X$ and $Y$ are representable. In this case, we can directly argue that the natural map
\begin{align*}
    \left| \Hom_{\DDelta}(-,[m]) \times \Hom_{\DDelta}(-,[n]) \right| \to \left| \Hom_{\DDelta}(-,[m]) \right| \times \left| \Hom_{\DDelta}(-,[n]) \right|
\end{align*}
is a homeomorphism.
\end{proof}

\begin{corollary}
Simplicial homotopies become honest homotopies of spaces after geometric realization.
\end{corollary}

\begin{remark}
We can study simplicial sets up to homotopy or spaces up to homotopy, and in a sense that can be made precise, these are essentially the same theory.
\end{remark}

\begin{remark}
This connection between simplicial sets and spaces emerged initially in a research program which used to be called ``combinatorial homotopy theory,'' led by Kan in the 1950's. % Goerss--Jardine p2
\end{remark}
...
\end{align*}
This latter category is called the category of \textit{simplicial presheaves}, or in higher category language it is just denoted $\Pre(\Var_k)$, and called the $\infty$-category of ($\infty$-)presheaves. It comes equipped with a homotopy theory coming levelwise from simplicial homotopy theory. It turns out this is not just a nice well-behaved way to access homotopy theory starting from $\Var_k$, it is actually the \textit{universal} way to do this (see e.g. \cite{Dugger-univ}).
...
\begin{note} If an algebraic group is linear, it is automatically affine (\cite[1.29]{MilneAG}) and finite type. Moreover, all linear algebraic groups are closed subgroups of $\GL_n$.
\end{note}
...
\begin{definition} \cite[pp.~5-11]{Grothendieck-torsion} A group scheme is called \textit{special} if it is a linear algebraic group $G$ with the property that every $G$-torsor\footnote{Since $G$ is affine, every $G$-torsor in $\tau\le \fppf$ is automatically a principal $G$-bundle.} is locally trivial in the Zariski topology.
\end{definition}

\begin{theorem} \cite[Th\'eor\`eme~3]{Grothendieck-torsion} The group $\GL_n$ is special.
\end{theorem}

\begin{corollary}[{\cite[11.4]{MilneLEC}}]
\label{cor:h1-gln-hilbert90} 
...
\begin{proposition}[{\cite[4.15,~12.18]{Joyal-qcats}}] If $\mathscr{C}$ is a quasi-category, then the map of simplicial sets
\begin{align*}
    \Fun(\Delta^2,\mathscr{C})\to \Fun(\Lambda_2^1,\mathscr{C})
\end{align*}
has contractible fibers (i.e., the geometric realization of the fibers under this map are contractible spaces). In fact $\mathscr{C}$ is a quasi-category if and only if this condition holds.\footnote{In Joyal's notes, this is phrased that the induced map is a \textit{trivial fibration}. We can fibrantly replace it by a Kan fibration, then by \cite[12.18]{Joyal-qcats} a Kan fibration is a trivial fibration if and only if its fibers are contractible.}
\end{proposition}
...
\begin{enumerate}
    \item \textit{Schemes} all live in here via the Yoneda embedding $\Sch_S \hookto \Fun(\Sch_S^\op, \Set) \subseteq \Fun(\Sch_S^\op, \sSet)$, by viewing $\Set \subseteq \sSet$ as discrete simplicial sets (no non-degenerate $n$-simplices for $n\ge 1$).
    ...
    \end{align*}
    %
    \item \textit{Spaces} (viewed as simplicial sets by $\Pi_\infty$) live in here as constant presheaves $\sSet \hookto \Fun(\Sch_S^\op, \sSet)$.
\end{enumerate}
...
\begin{example}[Affine distinguished Nisnevich squares]
\label{ex:affine-distinguished-nisnevich-square}
Suppose $f: R \to S$ is a finite \'etale ring homomorphism, and $x\in R$ is some element for which $R/x \cong S/f(x)$ is a ring isomorphism. Then we have a distinguished Nisnevich square:
\[ \begin{tikzcd}
...
\begin{exercise} Show that $\Mod(-)$ is a Nisnevich sheaf on the site of affine schemes. (In fact $\Mod(-)$ has much stronger descent properties as we will see in \autoref{thm:qcoh-fpqc-descent}).
\end{exercise}
...
\begin{notation}\label{nota:truncated-subcategory}
For any presentable $\infty$-category, we denote by $\mathscr{C}_{\le n}$ or $\tau_{\le n}\mathscr{C}$ the full subcategory spanned by the $n$-truncated objects.
\end{notation}
...
Let $f \colon X \to Y$ be a map of CW complexes. Then $f$ is a homotopy equivalence if and only if $f \colon \pi_i(X) \to \pi_i(Y)$ is an isomorphism for each $i\ge0$.
\end{theorem}
...
\begin{definition}
An $\infty$-topos is \textit{hypercomplete} if and only if every object is $\infty$-connective.
\end{definition}
...
We have two more notions we want to define -- the idea of \textit{homotopy dimension} and that of \textit{Postnikov completeness}.
...
We can also ask, for any $\infty$-topos, to what extent the category is determined by its truncations. More precisely, we make the following definition:

\begin{definition}[{\cite[A.7.2.1]{SAG}}]
Let $\mathscr{C}$ be any presentable $\infty$-category.

\begin{enumerate}
    \item We say it is \textit{Postnikov complete (SAG)} if the induced map
\begin{align*}
    \mathscr{C} \to \lim \left( \cdots \mathscr{C}_{\le 3} \xto{\tau_{\le 2}} \mathscr{C}_{\le 2} \xto{\tau_{\le 1}} \mathscr{C}_{\le 1}\xto{\tau_{\le 0}} \mathscr{C}_{\le 0} \right)
\end{align*}
is an equivalence.
    \item We say it is \textit{Postnikov complete (HTT)} if for every $X\in \mathscr{C}$, the induced map
    \begin{align*}
        X \to \lim \left( \cdots X_{\le 3} \xto{\tau_{\le 2}} X_{\le 2} \xto{\tau_{\le 1}} X_{\le 1}\xto{\tau_{\le 0}} X_{\le 0} \right)
    \end{align*}
    is an equivalence.
\end{enumerate}
The former definition implies the second, and it is still open as to whether the second implies the first. When we say ``Postnikov complete'' we will be referring to the first definition, unless otherwise specified.
\end{definition}

\begin{slogan} The notion of Postnikov completion in HTT should be thought of as the statement that ``Postnikov towers converge.'' In particular it should be considered as a green light for obstruction theory.
\end{slogan}


For $\infty$-topoi, all these notions are related via the following:

\begin{proposition}\label{prop:htpy-dim-Postnikov-complete-hypercomplete} 
Let $\mathscr{C}$ be an $\infty$-topos. Then the following implications hold:
\begin{align*}
    \left\{ \mathscr{C}\text{ has finite htpy dimension} \right\}\Rightarrow \left\{ \mathscr{C}\text{ is Postnikov complete (HTT)} \right\}\Rightarrow \left\{ \mathscr{C}\text{ is hypercomplete} \right\}.
\end{align*}
The first implication is \cite[7.2.1.10]{HTT}, the latter is basically by definition. %\cite[A.7.2.3]{HTT}
\end{proposition}

\begin{question} In \Cref{prop:htpy-dim-Postnikov-complete-hypercomplete}, can we replace the notion of Postnikov complete with the definition from SGA? The latter implication is clearly fine, so we're asking -- if $\mathscr{C}$ has finite homotopy dimension, is it Postnikov complete in the sense of SGA?
\end{question}

Our main use for the results above will be the following theorem of Voevodsky.

\begin{theorem} (Voevodsky) If $S$ is qcqs and Noetherian, then the homotopy dimension of $\Shv_\Nis(\Sm_S)$ is bounded above by $\dim(S)$. In particular this implies that Postnikov towers converge, and we have hypercompleteness.
\end{theorem}

Hence in our main setting, Nisnevich sheaves over a field, we can carry out obstruction theory arguments!
...
\end{enumerate}
We will revisit these definitions in \autoref{prop:strongly-strictly-redefined}.
\end{definition}
...
\begin{theorem}[{\cite[2.3]{Morel}}] Any strictly $\A^1$-invariant sheaf of abelian groups is unramified.
\end{theorem}
...
\[ \begin{tikzcd}
    M(\mathcal{O}_v)\dar[hook]\rar[dashed] & M(\mathcal{O}_w)\dar[hook]\\
    M(E)\rar & M(F).
\end{tikzcd} \]
...
By \autoref{thm:restriction-to-Fk}, we can define unramified sheaves of abelian groups via their values on fields. In the following few sections we provide some examples. We'll start with Milnor $K$-theory, but let's pause to motivate it and give some examples.

\subsection{Background: why Milnor $K$-theory?}

By the end of the 1960's, work on the lower algebraic $K$-groups of rings was in full swing. Around the end of this decade, some new developments occurred which would set the stage for the explosion of results in the 70's. Milnor defined $K_2$ of a ring in a course in 1967 in terms of Steinberg modules, and Matsumoto's 1968 thesis gave a presentation of $K_2$ of a field $F$ (see \Cref{sec:jonathan} for a full exposition of this). In particular, one has the following:

\begin{theorem}[{Matsumoto,~c.f.~\Cref{thm:matsumoto}}] If $F$ is a field, then $K_2(F)$ is generated as an abelian group by \emph{symbols} $\left\{ x,y \right\}$ for each $x,y\in F^\times$, subject to the relations
\begin{itemize}
    \item (bilinearity) we have that
    \begin{align*}
        \left\{ x_1x_2 ,y \right\}&= \left\{ x_1,y \right\} + \left\{ x_2 y \right\} \\
        \left\{ x,y_1y_2 \right\} &= \left\{ x,y_1 \right\} + \left\{ x,y_2 \right\}
    \end{align*}
    \item (Steinberg relation) we have
    \begin{align*}
        \left\{ x,1-x \right\} = 0
    \end{align*}
    for $x\ne 1$.
...
This had a wealth of applications -- chief among them is a new capacity to define abelian group homomorphisms \emph{out} of algebraic $K_2$ of a field, since we now have a presentation for it. One of the most profound was the development of the \emph{norm residue symbols} by Tate. These went from $K_2(F)$ to the Brauer group $H^2_\et(F,\mu_n^{\otimes2})$, but landed in the $n$-torsion elements, inducing a map
\begin{equation}\label{eqn:norm-residue}
\begin{aligned}
    K_2(F)/n \to \Br(F)[n].
\end{aligned}
\end{equation}
It is now a theorem of Merkurjev-Suslin and Rost that this is an isomorphism for all fields $F$. Milnor observed that the Hasse invariant of a quadratic form\footnote{This notation $I^n(F)$ is defined in \Cref{sec:milnor-conjectures}.} $I^2(F)/I^3(F) \xto{\sim} \Br(F)$ of a quadratic form factors through Tate's mod two norm residue map in \Cref{eqn:norm-residue}, inducing an isomorphism $K_2(F)/2 \cong I^2(F)/I^3(F)$. To that end, Milnor constructed a graded ring $K_\ast^M(F)$ attached to any field $F$, and provided symbol maps $K_n^M(F)/2 \to I^n(F)/I^{n+1}(F)$ (see \Cref{eqn:milnor-mod-2-to-powers-ideal}). Milnor denoted his graded ring by $K_\ast(F)$, suggestively indicating that it might be an elegant construction of the higher $K$-groups. We now call something different $K$-theory, and denote by $K_\ast^M(F)$ the graded ring Milnor constructed for a field $F$. We call this \emph{Milnor $K$-theory}.


\begin{definition}[{\cite{MilnorK}}]
\textit{Milnor $K$-theory} of a field $F$ is defined to be the graded ring $K_\ast^M$ generated by symbols $\left\{ a \right\}\in K_1^M$ for $a\in F^\times$ subject to the relations
\begin{enumerate}
...
\begin{proposition}\label{prop:Milnor-K-thy-continuous} 
For any $n$, and any base field $k$, we have that Milnor $K$-theory
\begin{align*}
    K_n^M \colon \mathcal{F}_k \to \Ab
\end{align*}
is continuous. That is, for any $F\in \mathcal{F}_k$, we have that the natural map is an isomorphism:
\begin{align*}
   \mathop{\colim}_{\substack{E \subseteq F \\ E \in \mathcal{E}_k}} K_n^M(E) \xto{\sim} K_n^M(F).
\end{align*}
\end{proposition}
\begin{proof} Let $F\in \mathcal{F}_k$. Then any symbol $\left\{ a_1, \ldots, a_n \right\}\in K_n^M(E)$ involves only \emph{finitely many elements} of $F$, hence lies in $k(a_1, \ldots, a_n)$, which is a finitely generated field over $k$. Similarly the Steinberg relation involves only one element, so it is detected on $\mathcal{E}_k$.
\end{proof}

\begin{corollary} Mod $m$ Milnor $K$-theory is also continuous; that is, for $F\in \mathcal{F}_k$ we have
\begin{align*}
   \mathop{\colim}_{\substack{E \subseteq F \\ E \in \mathcal{E}_k}} K_n^M(E)/m \xto{\sim} K_n^M(F)/m.
\end{align*}
\end{corollary}
...
\end{proposition}
\begin{proof} This is an immediate corollary of bilinearity of symbols. By \textbf{(M2)}, we have that $\left\{ 1\cdot 1 \right\} = \left\{ 1 \right\} + \left\{ 1 \right\}$, hence $0 = \left\{ 1 \right\}$ by subtracting a copy of $\left\{ 1 \right\}$ from either side.
\end{proof}

\begin{proposition}\label{prop:powers-symbols} 
In $K_2^M(F)$ we have that
\begin{enumerate}
    \item $-\left\{ a,b \right\} = \left\{ a^{-1},b \right\}$.
    \item More generally for any $i,j\in \Z$ we have
    \begin{align*}
         \left\{ a^i,b^j \right\} = (i\cdot j) \left\{ a,b \right\}.
    \end{align*}
\end{enumerate}
\end{proposition}
...
\begin{align*}
    \left\{ a,-a \right\} &= 0 \\
    \left\{ a,a \right\} &= \left\{ a,-1 \right\}.
\end{align*}
In particular $2\left\{ a,a \right\} = 0$ for any $x$.
\end{proposition}
\begin{proof} Note for $a\ne 1$, we have that
\begin{align*}
    -a = \frac{1-a}{ 1- a^{-1}}.
\end{align*}
%
Hence 
\begin{align*}
    \left\{ a,-a \right\} &= \left\{ a,1-a \right\} - \left\{ a, 1-a^{-1} \right\} = -\left\{ a, 1- a^{-1} \right\} = \left\{a^{-1}, 1-a^{-1} \right\} = 0.
\end{align*}
This last equality uses \autoref{prop:powers-symbols}.
The argument that $\left\{ a,a \right\} = \left\{ a,-1 \right\}$ is similar.
The last statement follows from observing that
\begin{align*}
    2\left\{ a,a \right\} = 2 \left\{ a, -1 \right\} = \left\{ a, 1 \right\} = 0,
\end{align*}
...
\end{align*}
\end{proposition}
\begin{proof} By induction it suffices to treat the case where $\alpha,\beta\in K_1^M(F)$. In this setting, we see that, for any two $a,b\in F^\times$, we have
\begin{align*}
    \left\{ a,b \right\} + \left\{ b,a \right\} &= \left\{ a,b \right\} + \left\{ a,-a \right\} + \left\{ b,-b \right\} + \left\{ b,a \right\} \\
    &= \left\{ a,-ab \right\} + \left\{ b,-ab \right\} \\
    &= \left\{ ab,-ab \right\} = 0.
\end{align*}
Hence $\left\{ a,b \right\} = - \left\{ b,a \right\}$.
\end{proof}

\textbf{More reading}: For more about symbol algebra in $K_\ast^M$, we refer the reader to \cite[\S7.1]{GilleSzamuely}.
...
\end{example}
\begin{proof}
It suffices to show that $K_2^M(\mathbb{F}_q)=0$. Pick $a$ to generate $\mathbb{F}_q^\times$, then any element in $K_2^M(\mathbb{F}_q)$ is of the form $\left\{ a^i,a^j \right\}$. By \autoref{prop:powers-symbols}, this is equal to $ij \left\{ a,a \right\}$ so it will suffice to verify that $\left\{ a,a \right\} = \left\{ a,-1 \right\}$ vanishes. This has order dividing two. We want to show it is killed by an odd number as well, and we'll be done. If $q = 2^m$, then $a^{2^m-1}=1$, and hence
\begin{align*}
    0 &= \left\{ 1,a \right\} = \left\{ a^{2^m-1},a \right\} = \left( 2^m-1 \right)\left\{ a,a \right\},
\end{align*}
which concludes the proof. If $q$ has odd eaponential characteristic, we can find two non-squares in $\mathbb{F}_q^\times$ which sum to 1 (c.f. \cite[1.3.6]{GilleSzamuely}). This gives us $a^k + a^\ell = 1$ for $k,\ell$ odd. Hence we get
\begin{align*}
    0 = \left\{ a^k,a^\ell \right\} = k\cdot \ell\left\{ a,a \right\},
\end{align*}
and we are done.
\end{proof}

\begin{remark}[Milnor $K$-theory isn't $K$-theory] Note that the above example immediately implies Milnor $K$-theory and Quillen $K$-theory disagree, since Quillen computes the algebraic $K$-groups of finite fields as
\begin{align*}
    K_n(\mathbb{F}_q) &=
    \begin{cases}
    \Z & n = 0 \\
    ...
    \end{cases}
\end{align*}
In particular, $K_3^M(\mathbb{F}_q) = 0$ but $K_3(\mathbb{F}_q) = \Z/(q-1)$.
\end{remark}
...
\subsection{Residue and specialization homomorphisms for Milnor $K$-theory}
Suppose $F$ is equipped with a discrete valuation $v$ and residue field $\bar{F}_v$, and denote by $\mathcal{O}_v \subseteq F$ the \emph{valuation ring}. We can pick a uniformizing parameter $\pi$ for which every element in $F$ is uniquely expressible as $\pi^r u$ for $u\in \mathcal{O}_v$ a \emph{unit} and $r\in \Z$. In this setting, the Milnor $K$-theory of $F$ is related to that of its residue field.

\begin{lemma}[{\cite[2.1]{MilnorK}}] In the situation above, there is a well-defined \emph{residue homomorphism}
\begin{equation}\label{eqn:Milnor-K-residue-homomorphism}
\begin{aligned}
    \del_v \colon K_{n+1}^M(F) \to K_n^M(\bar{F}_v),
\end{aligned}
\end{equation}
with the property that, for any uniformizing parameter $\pi$ and any units $u_i\in \mathcal{O}_v^\times$, we have that\footnote{Here we are conflating $u_i$ with its image in $\bar{F}_v$ --- we should probably write $\bar{u}_i$ instead as Milnor does.}
\begin{align*}
    \left\{ \pi, u_1, \ldots, u_n \right\} &\mapsto \left\{ u_1, \ldots, u_n \right\} \\
    \left\{ u_0,u_1, \ldots, u_n \right\}&\mapsto 0.
\end{align*}
\end{lemma}
\begin{proof} Omitted, see \cite[\S2]{MilnorK}.
\end{proof}

\begin{notation} It is clear that these residue homomorphisms in \Cref{eqn:Milnor-K-residue-homomorphism} are surjective, so we denote by
\begin{equation}\label{eqn:Milnor-K-thy-of-unit-ring}
\begin{aligned}
    K_n^M(\mathcal{O}_v) := \ker \left( K_n^M(F) \xto{\del_v} K_{n-1}^M(\bar{F}_v) \right) \in \Ab.
\end{aligned}
\end{equation}
A priori this is just notation --- $K_n^M(-)$ is defined on fields, so it doesn't even make sense to plug in a ring. We will see soon that $K_n^M$ is a prototypical example of an unramified $\mathcal{F}_k$-datum, hence it promotes to a \emph{sheaf}, which we denote $\KM_n$. After this we verify that $H^0(\Spec(\mathcal{O}_v),\KM_n) = K_n^M(\mathcal{O}_v)$ as above.% todo -- this should be like a Hilbert 90 thing
\end{notation}

\begin{definition}\label{def:specialization-Milnor-K-theory} 
For $F$ a field, and $v$ a discrete valuation, we have a natural \emph{specialization homomorphism}
\begin{align*}
    s_v \colon K_n^M(\mathcal{O}_v) &\to K_n^M(\bar{F}_v) \\
    \left\{ u_1, \ldots, u_n \right\}&\mapsto \left\{ u_1, \ldots, u_n \right\}.
\end{align*}
\end{definition}

\begin{proposition}\label{prop:A1-axiom-Milnor-K-thy} 
If $(E,v) \subseteq (F,w)$ is a separable extension of discretely valued fields where $v$ has ramification index $1$ on $E$, then the square commutes
\[ \begin{tikzcd}
    K_n^M(\mathcal{O}_v)\dar\rar & K_n^M(\mathcal{O}_w)\dar\\
    K_n^M(E)\rar & K_n^M(F),
\end{tikzcd} \]
and if $\bar{E}_v \to \bar{F}_w$ is an isomorphism, the above square is cartesian.
\end{proposition}
...
\subsection{The Milnor exact sequence}

The following is a key result we will make use of.

\begin{theorem}[{Milnor exact sequence,~\cite[2.3]{MilnorK}}] Let $F$ be an arbitrary field, and consider the field of rational functions $F(t)$. Then the sequence
\begin{align*}
    0 \to K_n^M(F) \to K_n^M(F(t)) \xto{\oplus\del_\pi} \bigoplus_{\substack{(\pi)\in \Spec(\A^1_F) \\ (\pi)\ne (0)}} K_{n-1}^M(F[t]/\pi) \to 0
\end{align*}
is split exact.
\end{theorem}
\begin{proof} See \cite[pp.~325-327]{MilnorK}.
\end{proof}


\begin{proposition}\label{prop:KM-is-an-unramified-sheaf} 
Milnor $K$-theory gives rise to an unramified sheaf of abelian groups $\KM_n$ for every $n$.
\end{proposition}
\begin{proof} By \Cref{thm:restriction-to-Fk} it suffices to check that Milnor $K$-theory $K_n^M(-)$ defines an unramified $\mathcal{F}_k$-datum for every $n$, hence we check that the conditions of \Cref{def:unramified-Fk-datum} are satisfied. The continuity of Milnor $K$-theory (\textbf{D1}) is \Cref{prop:Milnor-K-thy-continuous}, our subgroups $K_n^M(\mathcal{O}_v) \subseteq K_n^M(F)$ is \Cref{eqn:Milnor-K-thy-of-unit-ring}, and specialization morphisms in \Cref{def:specialization-Milnor-K-theory}. We verified \textbf{A1} in \Cref{prop:A1-axiom-Milnor-K-thy}, so it suffices to check the remaining axioms (todo).
\end{proof}
...
\begin{enumerate}
    \item[\textbf{(MW4)}] $\eta h = 0$
    \item[\textbf{(MW4)}] $\epsilon \eta = \eta$.
\end{enumerate}
\end{remark}
...
\label{prop:KMW-unramified-and-invariant} 
...
\section{The Milnor conjectures}\label{sec:milnor-conjectures}
...
We can obtain unramified sheaves of groups
\begin{align*}
    \mathbf{GW},\ \mathbf{W},\ \mathbf{I}^j,
\end{align*}
associated to each of these $\mathcal{F}_k$-data. There is a tremendous amount to check here. The statement for Witt sheaves is \cite[Corollary~6]{Morel-connectivity}, while the statement for $\mathbf{GW}$ follows from \Cref{prop:KMW-unramified-and-invariant} when $n=0$.
...
\begin{example}[Examples of fundamental ideals] \,
\begin{enumerate}
    ...
    \end{align*}
    %
    This is a result of Pfister (c.f. \cite[II.2.3]{Lam-quad}).
...
\end{align*}
since we can show that if $\alpha - \beta \in \bigcap_{n=0}^\infty I^n(F)$, then $\alpha = \beta$. The question of whether this intersection is zero was first raised by Milnor \cite[4.4]{MilnorK}, and proven shortly thereafter by Aarason and Pfister as a consequence of their Hauptsatz \cite[X.5.1]{Lam-quad}.
\end{proof}




\begin{proposition}[{\cite{MilnorK}}] There is a homomorphism
...
\begin{proposition}[Properties of $K(A,n)$] \,
\begin{enumerate}
...
With Eilenberg-MacLane spaces in hand in the sheaf topos, we can reframe our definitions of strong and strict invariance.
...
\begin{proposition}[Properties of contraction] See e.g. \cite[\S4.1]{bachmannstrongly}
\begin{enumerate}
...
\begin{theorem}[Properties of the Rost--Schmid complex] \,
\begin{enumerate}
...
\begin{theorem}[{\cite[p.~323]{Rost-Chow}}] For any smooth $X$, we have that $H^n(X,\KM_n) = \CH^n(X)$.
\end{theorem}
...
\begin{remark}[Vanishing of $I^j$-cohomology] \,
\begin{enumerate}
...
\begin{theorem}[{\cite{Cazanave-thesis}}] The B\'{e}zoutian is a symmetric bilinear matrix, equal to the $\A^1$-degree of $f/g$, and the bijection
\begin{align*}
...
\begin{theorem}[{\cite{Cazanave}}] The set $\left[ \P^1,\P^1 \right]_N$ of pointed naive homotopy classes of maps admits a monoid structure, and the natural map
\begin{align*}
    \left[ \P^1,\P^1 \right]_N \to \left[ \P^1,\P^1 \right]_{\A^1}
\end{align*}
is a group completion.\footnote{Interesting, this seems to be special to $\P^1$. Such a statement is never true for punctured affine space, for instance (see \cite{BrazeltonHornslien}).}
\end{theorem}
...
\begin{theorem}[{Unstable connectivity theorem, \cite[Thm~6.38]{Morel}}] 
...
\begin{definition} Take a collection of generators for $\pi_0 M$, and denote by $M_\infty$ the colimit of multiplying by finite subsets of the generators infinitely many times. In nice settings, this is precisely $M \left[ \pi_0(M)^{-1} \right]$.
\end{definition}
...
\begin{theorem}\label{thm:K-is-BGLxZ} 
We have that $\Z \times \BGL \to K$ is a motivic equivalence.
\end{theorem}
...
The cyclic condition reduces to checking that $\begin{pmatrix} 0 & 0 & 1 \\ 1 & 0 & 0 \\ 0 & 1 & 0 \end{pmatrix}$ acts trivially on $\mathcal{O}^{\oplus3}$ after motivic localization, which is immediate.
\end{proof}
...
\begin{theorem}[{\cite{TT}}] Algebraic $K$-theory is a Nisnevich sheaf of spectra.
\end{theorem}
...
\begin{corollary}[{see \cite[6.15]{AE}}]
We have that
\begin{align*}
    \pi_i \BGL_n = K_i
\end{align*}
for $2\le i\le n-1$.\footnote{todo - reorder this result so we don't have forward references}
\end{corollary}
\begin{proof} We have an $\A^1$-fiber sequence \Cref{eqn:fiber-stabilization-BGLr}, hence we get a long exact sequence of homotopy sheaves
\begin{align*}
    \cdots \to \pi_i(S^{2r-1,r}) \to \pi_i(\BGL_{r-1}) \to \pi_i (\BGL_r) \to \pi_{i+1}(S^{2r-1,r}) \to \cdots 
\end{align*}
Since $S^{2r-1,r}$ is $\A^1$-$(r-2)$-connected, the suspension map $\pi_i(\BGL_{r-1}) \to \pi_{i}(\BGL_{r})$ is an isomorphism for $i\le r-2$. Letting $n = r-1$, we see that if $i\le n-1$, we have that
\begin{align*}
    \pi_i(\BGL_n) \cong \pi_i(\BGL_{n+1}) \cong \cdots \cong \pi_i(\BGL).
\end{align*}
Since $K \simeq \Z \times \BGL$ by \Cref{thm:K-is-BGLxZ}, we have that this is equal to $K_i$.
\end{proof}
...
\begin{align*}
    X = \tau_{\le \infty} X = \lim_n \tau_{\le n} X.
\end{align*}
%
...
\end{tikzcd} \]
into a sequence of analogous stages. This relative situation is often called the \textit{Postnikov--Moore tower} attached to the map $Y \xto{f} Z$. Its theory is outlined in \cite[\S B]{Morel}. The key point is that if $f\colon Y \to Z$ is a fibration with fiber $F$, then the $k$-invariants for lifting $X \to Z$ along $f$ are valued in Eilenberg--MacLane spaces for the homotopy sheaves of $F$. Our primary application of this is the classification of torsors.
...
\begin{theorem}[{Affine representability \cite{AHW1}}]
If $X \in \Sm_k^\aff$ is any smooth affine $k$-scheme, then there is a bijection
\begin{align*}
...
\begin{theorem}
If $X = \Spec(R)$ is a smooth affine $k$-scheme of dimension 2 over an algebraically closed field, then a rank two finitely generated projective module over $X$ is completely determined by its $c_2$.
\end{theorem}
...
\end{align*}
So the number of algebraic vector bundles over $X$ with fixed $c_2$ is acted upon transitively by $H^2(X,\mathbf{I}^3)$. If $k = \bar{k}$, this set vanishes \cite[5.1]{AF3}.
\end{proof}

If $k$ is not algebraically closed, $H^2(X,\mathbf{I}^3)$ need not vanish, and it may in fact contain more data governing the structure of algebraic vector bundles. An explicit example is the following:

\begin{example}[{\cite[5.12]{Fasel-unim}}]
\label{exa:stably-free-corank-zero-algebraic-S2}
Let $X$ the algebraic 2-sphere $X=\Spec(A)$, where
\begin{align*}
    A = \frac{\R[x,y,z]}{x^2 + y^2 + z^2 - 1}.
\end{align*}
Then there are infinitely many non-isomorphic rank two algebraic vector bundles with vanishing $c_1$ and $c_2$.
\end{example}

More generally, if $A\in \Sm_\R^\aff$ is a smooth $\R$ algebra of dimension $2d$ which is rational and has trivial canonical bundle. Then by \cite{Fasel-unim}, we have an isomorphism
\begin{align*}
    H^{2d}(X, I^{2d+1}) \cong \oplus_{\pi_0(X(\R))}\Z.
\end{align*}
In particular, this is the number of stably free corank zero modules up to isomorphism!
...
\begin{theorem}[Motivic Swan-Forster]
\label{thm:motivic-Swan-Forster} 
If $A$ is a smooth $k$-algebra of Nisnevich cohomological dimension $\le d$, then every finitely generated projective $A$-module of rank $r$ needs at most $r+d$ generators.
\end{theorem}
...
\begin{theorem}[{Serre splitting \cite[Theorem~1]{Serre-dubreil58}}]
\label{thm:Serre-splitting} 
Let $R$ be a Noetherian ring of dimension $d$, and $M$ a projective $R$-module of rank $r > d$. Then $M$ decomposes as $M \cong M' \oplus R$ for some projective module $M'$.
\end{theorem}
...
\begin{theorem}[Motivic Serre splitting] Let $X = \Spec(R)\in \Sm_k^\aff$ be of \textit{Nisnevich cohomological dimension} $d$, and let $M$ be any finitely generated projective $R$-module. If $\text{rank}(M) > d$, then $M$ splits.
\end{theorem}
...
While the 2-cohomological dimension doesn't appear in the algebraically closed context, it can obstruct splitting over non-algebraically closed fields, where the group $H^r(X,\mathbf{I}^{r+1})$ need not vanish. We saw an example of this in \Cref{exa:stably-free-corank-zero-algebraic-S2}. For more on this story we refer the reader to \cite{AF2}.
...
\end{tikzcd} \]
%
The $k$-invariants as we have seen live in the (twisted) cohomology of the fiber, where the twist is by $\pi_1(\BGL_d)$, giving $\det E$. That is, our $k$-invariants are
\begin{align*}
    k_t \in H^{t+2}(X,\pi_{t+1}(S^{2d-1,d})(\det E)).
\end{align*}
%
Since $X$ is Krull dimension $d$, the cohomology vanishes for $t+2 \ge d+2$, hence:
\begin{itemize}
    \item $k_d$ and the higher $k$-invariants are all zero.
\end{itemize}
Moreover, by the connectivity of $S^{2d-1,d}$, we have that $\pi_{t+1}(S^{2d-1,d})$ vanishes for $t+1\le d-2$, hence:
\begin{itemize}
    \item $k_{d-3}$ and the lower $k$-invariants vanish.
\end{itemize}
This leaves us with two interesting obstruction classes:
\begin{enumerate}
